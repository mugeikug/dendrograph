import { isTerminal, plainText, type TreeNode } from './treeModel'
...
function measureLabelWidth(node: TreeNode, opts: LayoutOptions): number {
  ...
  const text = plainText(node.label)
  return Math.max(opts.minNodeWidth, opts.measureText(text, opts.fontSize) + 12)
}

function measureYieldWidth(node: TreeNode, opts: LayoutOptions): number {
  const text = plainText(node.triangleYield)
  return Math.max(opts.minNodeWidth, opts.measureText(text, opts.fontSize) + 12)
}
...
function toLayoutNode(
  ...
  const ln: LayoutNode = {
    ...
    y: depth * opts.rowHeight,
    width: local.width,
    depth,
    children: local.children.map((c) => toLayoutNode(c, depth + 1, offsetX, opts, nodesByPath)),
  }
...
export function layoutTree(tree: TreeNode, options: Partial<LayoutOptions> = {}): LayoutResult {
  ...
  const root = toLayoutNode(localRoot, 0, offsetX, opts, nodesByPath)
  ...
  for (const n of nodesByPath.values()) {
    const labelBottom = n.y + opts.fontSize
